import { createStore, runDailyDigest, runWeeklyReview } from "../../growth-kit/lib";
import { loadTenants, resolveGrowthConfig } from "./tenants";
...
async function forEachTenant(
  fn: (cfg: ReturnType<typeof resolveGrowthConfig>) => Promise<string>,
): Promise<RunResult[]> {
  const tenants = await loadTenants();
  const results: RunResult[] = [];
  for (const t of tenants) {
    try {
      const detail = await fn(resolveGrowthConfig(t));
...
export function runAllDailyDigests(): Promise<RunResult[]> {
  return forEachTenant(async (cfg) => {
    const d = await runDailyDigest(cfg, createStore(cfg));
...
export function runAllWeeklyReviews(): Promise<RunResult[]> {
  return forEachTenant(async (cfg) => {
    const review = await runWeeklyReview(cfg, createStore(cfg));
